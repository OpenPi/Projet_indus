...
class Thermocouple(AnalogSensor):
	...
	# Builder class
	def __init__(self, hardwareId, channel, refVolt, fSave, fRead, offset):
		"""
	...
	def conversion(self, voltValue):
		...
		temp = (voltValue - 1.25)/0.005 + self.offset
	...
	def get_value(self):
		...
		if voltValue == 0.0:
			#TODO Alert thermocouple malfunction and stop Heater
			Queue_Global.process_TemperatureRegulation.enqueue('Stop')
...
class PhMeter(AnalogSensor):
	...
	def get_value(self):
		...
		if voltValue == 0.0:
			#TODO Alert ph meter malfunction
			Queue_Global.process_PhRegulation.enqueue('Stop')
...
class AmpereMeter(AnalogSensor):
	...
	def conversion(self, value):
		...
		#print(value)
		# P = U*I
		wattValue = self.voltage*(value*30)
		return wattValue
