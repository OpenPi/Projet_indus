...
class PhRegulation(object):
	...
	def __init__(self):
		...
		self.set_point=float(database.databasePhRegulation.getUserConfigurationValue("ph_setpoint")) #set_point is requested setpoint 
	...
	def BD_PH(self):
		...
		"""
	    	SensorValuePH = database.databasePhRegulation.getLastMeasureByName("Pool Ph Meter")[0] 
	...
	def Calculate_Volume_Produced_LessPh(self, current_value_ph, set_point_ph):
		...
		#Variable
		varPoolVolume = self.poolVolume / 10.0
	...
	def Regulate(self):
		...
		for i in range(0, len(self.pumpDecision)):
