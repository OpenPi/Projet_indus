...
def set_pullUp(pin, value=True):
	...
	if value:
		io.set_pin_pullup(pin, 1)	# Set pin to pullUP
	else:
		io.set_pin_pullup(pin, 0)	# Set pin to pullDown
...
def get_input_num(pin):
	...
	return io.read_pin(pin)		# Read state of pin (0 or 1)	# TODO sent alert malfunction sensor
